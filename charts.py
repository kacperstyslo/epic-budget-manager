...
class ChartsMain(BudgetMain):
    ...
    def draw_savings_single_line_chart(self, **kwargs: list):
        ...
        savings_single_line_fig = plt.Figure(figsize=(6, 6.8), dpi=100, facecolor='#808080')
        savings_single_line_ax = savings_single_line_fig.add_subplot(111, facecolor='#808080')
        ...
        savings_single_line_ax.set_title('Savings 2021', fontweight='bold', color='#ffffff')
        savings_single_line_ax.set_ylim(ymin=0)
        savings_single_line_ax.yaxis.grid()
        savings_single_line_ax.tick_params(colors='#ffffff', which='both')
        savings_single_line_fig.tight_layout()

    def draw_pie_chart(self, **kwargs: list):
        pie_fig = plt.Figure(figsize=(8, 6), dpi=100, facecolor='#808080')
        ...
        explode_value = (0, .1)
        pie_ax.pie(pie_values, labels=('Savings', 'Expense'), explode=explode_value, shadow=True, autopct='%1.1f%%', colors=['green', 'red'], startangle=90, wedgeprops={"edgecolor": "k", 'linewidth': 2}, textprops={'color': "w"})
        pie_ax.axis('equal')
        pie_ax.set_title('Budget 2021', fontweight='bold', color='#ffffff')

    def draw_savings_expenses_bar_chart(self, savings_values_bar_chart, expenses_values_for_bar_chart):
        bar_savings_expenses_fig = plt.Figure(figsize=(6, 8), dpi=100, facecolor='#808080')
        bar_savings_expenses_ax = bar_savings_expenses_fig.add_subplot(facecolor='#808080')
        ...
        bar_savings_expenses_ax.set_title("Savings & Expenses", fontweight='bold', color='#ffffff')
        bar_savings_expenses_ax.axhline(y=0, linewidth=2, color='k')
        bar_savings_expenses_ax.tick_params(colors='#ffffff', which='both')
        bar_savings_expenses_ax.bar('Savings', savings_values_bar_chart, width=0.5, label='Savings', color='green', edgecolor="black", linewidth=2)
        bar_savings_expenses_ax.bar('Expenses', [x * (-1) for x in expenses_values_for_bar_chart], width=0.5, label='Expenses', color='red', edgecolor="black", linewidth=2)

    def draw_data_table(self, data_for_table: list):
        data_table_fig = plt.Figure(figsize=(7, 2), dpi=100, facecolor='#808080')
        ...
        data_table_ax.set_facecolor('#808080')
        data_table_ax.set_title("Latest Revenue and Expenses", fontweight="bold", color='#ffffff')
        # cell_color = ['#808080' for _ in range(len(data_for_table))]
        data_table_ax.table(cellText=dt.values, colLabels=dt.columns, loc='center', colColours=['yellow'] * 2)  # cellColours=([*cell_color], [*cell_color]))

    def show_savings_and_expenses_buttons(self):
        buttons_font_size = font.Font(size=25)
        savings_button = Button(self.root, command=self.add_income_to_budget, text='+', width=26, bg='#00cc00', fg='#ffffff', borderwidth=10)
        expenses_button = Button(self.root, command=self.add_expense_to_budget, text='-', width=26, bg='#FF0000', fg='#ffffff', borderwidth=10)
        savings_button.place(x=400, y=950)
